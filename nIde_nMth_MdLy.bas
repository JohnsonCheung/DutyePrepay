...
Function MdLyToMthBEIdxAy_All(MdLy$(), Optional FmI&) As Variant()
Dim DrAy()
Dim U&
    DrAy = MdLy_ToMthTy_BIdx_DrAy_All(MdLy, FmI)
    U = UB(DrAy)
...
For J = 0 To UB(MdLy)
    If SrcLinIsMth(MdLy(J)) Then
        ...
        M = MthBrkNew(L)
        If MthBrkIsMatch(M, MthNm, PrpTy) Then
            Push O, Array(J, M.Ty)
        End If
    End If
Next
...
Private Function MdLy_ToMthTy_BIdx_DrAy_All(MdLy$(), Optional FmI&) As Variant()
Dim J&, M As MthBrk
Dim Sel As Boolean
Dim O(), L$
For J = 0 To UB(MdLy)
