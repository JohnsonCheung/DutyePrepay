...
Function MthStruAy(MthNm$, Optional PrpTy$, Optional A As CodeModule) As MthStru()
Dim ALy$()
Dim ABEIdxAy()
...
Function MthStruAy_All(Optional A As CodeModule) As MthStru()
Dim ALy$()
Dim ABEIdxAy()
    Dim Md As CodeModule:
    Dim FmI&
    ...
    FmI = Md.CountOfDeclarationLines
    ALy = MdLy(Md)
    ABEIdxAy = MdLyToMthBEIdxAy_All(ALy, FmI)  ' By = BIdx array   ! Bidx array of given-module-A
...
With A.Brk
    ...
    D = DrAyNewKVByFldAp("BIdx EIdx MthNm Mdy MthTy PrpTy RetTyChr RetAs PrmStr", _
            A.BEIdx(0), A.BEIdx(1), .Nm, .Mdfy, .Ty, .PrpTy, .RetTyChr, .RetAs, .PrmStr)
End With
...
With A.Brk
    ...
    Case "Friend": M = 1
    Case "Private": M = 2
    Case Else: Er "{Mfy} is invalid", .Mdfy
    End Select
End With
